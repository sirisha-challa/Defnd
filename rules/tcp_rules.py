...
class TCPStateRule(TCPRule):
    ...
    This rule works with the DEfnTracker.  It takes two arguments:
    ...
    def add_connection(self, defnd_packet):
        """
        Add a connection to the Rule. Connections are represented as the TCP
        4-tuple. See the TCPPacket class for more info.
        """
        self._existing_connections.add(defnd_packet.to_tuple())

    def filter_condition(self, defnd_packet):
        if not TCPRule.filter_condition(self, defnd_packet):
            return False
        pipe = get_pipe()
        pipe.send(to_tuple(defnd_packet))
        state = pipe.recv()
        if self.match_if:
